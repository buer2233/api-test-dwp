...
本仓库**不是普通应用项目**，而是一个 **Codex Skill**（`api-test-dwp`），安装在 `~/.codex/skills/` 下。它的工作是驱动另一个消费方项目（`test-automation/E10自动化/接口自动化测试`）里的接口测试编写。绝大多数"源代码"是 Markdown 规范，只有 `tools/` 和 `capture/` 目录里有 Python。
