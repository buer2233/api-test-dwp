...
[api-test-dwp] output=...\capture\latest.jsonl
...
4. 查看 `capture/latest.jsonl`：
   - Windows 命令行：`type latest.jsonl`
   - 若看到多行 JSON 即成功
...
| 文件 | 说明 | 是否纳入 git |
|---|---|---|
| `capture/latest.jsonl` | 抓包落盘 | 否（.gitignore） |
| `tools/page_api_index.json` | page_api 索引 | 否（.gitignore） |
| `capture_selection.md` | 勾选草稿 | 否（.gitignore） |
